<?php
    ...
    $OrderBy    = !empty($_POST['OrderBy']) ? $_POST['OrderBy'] : "datetime";
    ...
    curl_setopt_array($curl, array(
        CURLOPT_URL => ''.$base_url.'/_Api/ListNewslater.php',
    ...
    if($response_arry['status']!=="success"){
        ...
        //Jika Berhasil
        $JmlHalaman=$response_arry['total_pages'];
        $data=$response_arry['data'];
        if(empty(count($data))){
            echo '
                <tr>
                    <td colspan="5" align="center">
                        <small class="text-danger">Belum Ada Data Newslater Pada Database Website</small>
                    </td>
                </tr>
            ';
            exit();
        ...
        foreach($data as $list){
            $id_newslater=$list['id_newslater'];
            ...
            echo '
                <tr>
                    <td align="left">
                        <input class="form-check-input item_check" type="checkbox" name="item_check" value="'.$email.'">
                    </td>
                    <td align="left"><small>'.$datetime.'</small></td>
                    <td align="left"><small>'.$nama.'</small></td>
                    <td align="left"><small>'.$email.'</small></td>
                    <td align="left">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end dropdown-menu-arrow">
                            ...
                            <li>
                                <a class="dropdown-item" href="javascript:void(0)" data-bs-toggle="modal" data-bs-target="#ModalEdit" data-id="' . $id_newslater . '" data-mode="Refresh">
                                    <i class="bi bi-pencil"></i> Kirim Email
                                </a>
                            </li>
                            <li>
                                <a class="dropdown-item" href="javascript:void(0)" data-bs-toggle="modal" data-bs-target="#ModalHapus" data-id="' . $id_newslater . '" data-mode="Refresh">
...
<script>
    ...
    $(document).ready(function(){
        // Ketika checkbox "CheckAll" di klik
        $('#CheckAll').on('change', function() {
            $('.item_check').prop('checked', this.checked);
        });

        // Jika salah satu item tidak dicentang, otomatis hilangkan centang di "CheckAll"
